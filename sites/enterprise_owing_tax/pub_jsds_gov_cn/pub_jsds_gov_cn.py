...
class PubJsdsGovCnSpider(TaskBase):
    def __init__(self):
        ...
        self.base_url = "http://pub.jsds.gov.cn/module/jslib/jquery/jpage/dataproxy.jsp?perpage=200&col=1&appid=1&webid=1&path=%2F&columnid={}&unitid=73729&webname=%E6%B1%9F%E8%8B%8F%E7%9C%81%E5%9C%B0%E6%96%B9%E7%A8%8E%E5%8A%A1%E5%B1%80&permissiontype=0"

    def start(self):
        for i in xrange(48038, 48053):
            while True:
                base_url = self.base_url.format(i)
            ...
            for ear_url in detail_urls:
                # ear_url = 'http://pub.jsds.gov.cn/art/2017/7/15/art_48038_853594.html'
                # ear_url = 'http://pub.jsds.gov.cn/art/2017/7/15/art_48038_853594.html'
                # ear_url = 'http://pub.jsds.gov.cn/art/2017/7/15/art_48039_852674.html'
    ...
    def get_detail_urls(self, response):
        ...
        if tree is None:
            self.logger.error('{} 列表页请求内容异常，请检查requests或url'.format(response.url))
            return []
    ...
    def parse(self, response):
        ...
        if href:
            ...
            for per_data in data[1:]:
                pre_per_row = per_data.xpath("./td/p")
                if not pre_per_row:
                    pre_per_row = per_data.xpath("./td")
                if len(pre_per_row) <= 2:
                    continue  # 补丁
                per_row = [value.xpath("string(.)") for value in pre_per_row]
                if len(per_row) >= 6:
                    main_td = per_row
                elif len(per_row) <= 4:
                    per_row.insert(0, main_td[0])
                    per_row.insert(1, main_td[1])
                    per_row.insert(2, main_td[2])
                    per_row.insert(3, main_td[3])
                try:
                    parse_data = {
