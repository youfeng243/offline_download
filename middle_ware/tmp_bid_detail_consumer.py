...
class BaseWorker(object):
    ...
    def bid_detail_handler(self):
        try:
            cursor = self.schedule_db['tmp_09_25_offline_crawl_data_bid_detail'].find({})
            for mq_data in cursor:
                try:
                    href = mq_data['PageUrl']
                    title = mq_data['title']
                    content = mq_data['content']
                    province = mq_data.get('province','')
                    city=mq_data.get('city','')
                    host=get_tld(href)
                    publish_time = mq_data['publish_time']
                    url = "http://www.gsei.com.cn/html/1336/2017-09-25/content-181194.html"
                    extract_data = {
                        "title": title,
                        "publish_time": publish_time,
                        "content": content,
                        "province": province,
                        "_site_record_id": host,
                        "href": href
                    }
...
if __name__ == '__main__':
    ...
    while True:
        try:
            base_worker = BaseWorker(logfile=logfile['test.logs'])
            data = {}
            base_worker.bid_detail_handler()
        except Exception as e:
